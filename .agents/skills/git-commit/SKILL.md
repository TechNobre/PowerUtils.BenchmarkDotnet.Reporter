---
name: git-commit
description: "Create high-quality git commits that follow Conventional Commits and semantic-release best practices. Inspect the working tree, split changes into logical commits, stage precisely, and write well-formed conventional commit messages (type(scope): subject, body, footers). Use when the user asks to commit, craft a commit message, stage changes, split work into multiple commits, mentions \"/commit\", or wants messages that drive semantic-release version bumps."
license: MIT
allowed-tools: Bash
---
...
1. **Inspect the working tree before staging.**
   - `git status` (or `git status --porcelain` for machine-readable output).
   - Analyze the diff to decide type/scope: use the staged diff when files are already staged, otherwise the working-tree diff.
     ```bash
     # If files are staged, use the staged diff
     git diff --staged

     # If nothing staged, use the working tree diff
     git diff
     ```
   - For large diffs use `git diff --stat` first.
...
3. **Stage only what belongs in the next commit.**
   ...
   # Patch staging for mixed changes
   git add -p
   ```
   - **Never stage or commit secrets** (`.env`, `credentials.json`, private keys, tokens).
...
6. **Write and execute the commit** following the format above.
   - Choose the type deliberately (it controls semantic-release version bumps).
   - Single-line subject:
     ```bash
     git commit -m "<type>(<scope>): <subject>"
     ```
   - Multi-line with body/footer, use a heredoc (or `git commit -v` in an editor):
     ```bash
...
- **NEVER** run destructive commands (`--force`, `git reset --hard`) without an explicit request.
- **NEVER** skip hooks (`--no-verify`) unless the user explicitly asks.
- **NEVER** force-push to `main`/`master`.
- If a commit fails due to hooks, fix the issue and create a **new** commit, don't amend published history.
